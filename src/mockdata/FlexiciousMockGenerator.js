...
import { UIUtils, Constants, EventDispatcher, DateUtils } from '../js/library'
...
export default class FlexiciousMockGenerator extends EventDispatcher {
    ...
    init() {
        this.deepList = [];
        this.index = 0;
        // const timer = new flexiciousNmsp.Timer(100);
    ...
    onTimer(evt) {
        for (let i = _pageIndex * _pageSize; i < Math.min(FlexiciousMockGenerator.companyNames.length, (_pageIndex + 1) * _pageSize); i++) {
    ...
    getPagedOrganizationList(filter) {
        ...
        if (filter.implementsOrExtends('PrintExportFilter')) {
            ...
            if (pef.printExportOptions.printExportOption == flexiciousNmsp.PrintExportOptions.PRINT_EXPORT_ALL_PAGES) {
                ...
                return new flexiciousNmsp.PagedResult((UIUtils.filterPageSort(this.deepList, filter)), filter.recordCount);
            } else {
                return new flexiciousNmsp.PagedResult((UIUtils.filterPageSort(this.deepList, filter, [pef.printExportOptions.pageFrom, pef.printExportOptions.pageTo])), filter.recordCount);
            }
        }
        else
        ...
        for (const org of this.getDeepOrgList()) {
            if (org.id == orgId) {
                ...
                return new flexiciousNmsp.PagedResult(filter.pageIndex >= 0 ?
                    (UIUtils.filterPageSort(arr, filter)) : arr, arr.length,
    ...
    createOrganization(legalName, deep) {
        if (deep) this.index++;
        const org = new flexiciousNmsp.CustomerOrganization();
        ...
        if (deep) {
            for (let dealIdx = 0; dealIdx < FlexiciousMockGenerator.DEALS_PER_ORG; dealIdx++) {
    ...
    createDeal(idx, org, deep) {
        ...
        const deal = new flexiciousNmsp.Deal();
        deal.customer = org;
        ...
        if (deep) {
            ...
            for (let invoiceIDx = 0; invoiceIDx < FlexiciousMockGenerator.INVOICES_PER_DEAL; invoiceIDx++) {
    ...
    createInvoice(idx, deal, deep) {
        const invoice = new flexiciousNmsp.Invoice();
        invoice.deal = deal;
        ...
        if (deep) {
            for (let lineItemIDx = 0; lineItemIDx < FlexiciousMockGenerator.LINEITEMS_PER_INVOICE; lineItemIDx++) {
    ...
    static createContact() {
        const commercialContact = new flexiciousNmsp.Person();
    ...
    static getRandomDate() {
        return new Date(FlexiciousMockGenerator.getRandom(2015, 2017), FlexiciousMockGenerator.getRandom(0, 11), FlexiciousMockGenerator.getRandom(1, 28));

    }

    static generatePhone() {
        return `${FlexiciousMockGenerator.areaCodes[FlexiciousMockGenerator.getRandom(0, 3)]}-${FlexiciousMockGenerator.getRandom(100, 999).toString()}-${FlexiciousMockGenerator.getRandom(1000, 9999).toString()}`;
    ...
    static getSystemUser() {
        ...
        const user = new flexiciousNmsp.SystemUser();
        user.addedBy = user;
    ...
    static createAddress() {
        const address = new flexiciousNmsp.Address();
...
FlexiciousMockGenerator.dpHierarchyXML = '<root><Region Region="Southwest">' +
